...
def agregar_parada():
    ...
    c = conn.cursor()
    c.execute("INSERT INTO paradas (nombre, direccion, latitud, longitud) VALUES (?, ?, ?, ?)",
...
def obtener_paradas():
    conn = sqlite3.connect('paradas.db')
    c = conn.cursor()
    c.execute("SELECT nombre, latitud, longitud FROM paradas")
    paradas = c.fetchall()
    conn.close()
...
def calcular_ruta_optima():
    ...
    conn = sqlite3.connect('paradas.db')
    c = conn.cursor()
    c.execute("SELECT latitud, longitud FROM paradas")
    paradas = c.fetchall()
    conn.close()

    # Función para calcular la distancia entre dos puntos usando OSRM
    ...
    def encontrar_ruta_optima(origen, paradas):
        ruta_optima = []
        paradas_restantes = paradas.copy()

        # Comenzar desde la ubicación actual
        punto_actual = origen
        while paradas_restantes:
            # Encontrar la parada más cercana
            parada_mas_cercana = min(
                paradas_restantes,
                key=lambda parada: calcular_distancia(punto_actual, parada)
            )
            ruta_optima.append(parada_mas_cercana)
            paradas_restantes.remove(parada_mas_cercana)
            punto_actual = parada_mas_cercana

        return ruta_optima

    # Encontrar la ruta óptima
    ruta_optima = encontrar_ruta_optima((lat_actual, lon_actual), paradas)

    # Formatear las coordenadas para OSRM (ubicación actual + paradas ordenadas)
    coordenadas = f"{lon_actual},{lat_actual};" + ";".join([f"{parada[1]},{parada[0]}" for parada in ruta_optima])
